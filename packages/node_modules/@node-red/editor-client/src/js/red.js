...
var RED = (function() {
    ...
    function completeLoad(showProjectWelcome) {
        ...
        RED.comms.subscribe("canvas-text/#",function(topic,msg) {
            var parts = topic.split("/");
            var node = RED.nodes.node(parts[1]);
            if (node) {
                if (msg && msg.hasOwnProperty("text")) {
                    node.canvasText = msg;
                } else {
                    delete node.canvasText;
                }
                node.dirty = true;
                RED.view.redraw();
            }
    ...
    function buildMainMenu() {
        ...
        $('<li><a id="red-ui-header-button-sidemenu" class="button" href="#" aria-label="'+RED._('menu.label.mainMenu')+'"><i class="fa fa-bars"></i></a></li>').appendTo(".red-ui-header-toolbar")
        ...
        $.getJSON(options.apiRootUrl+"theme", function(theme) {
            if (theme.header) {
                ...
                if (theme.header.image) {
                    $('<img>',{src:theme.header.image, alt:theme.header.title||""}).appendTo(logo);
                }
                if (theme.header.title) {
                    $('<span>').attr('title', theme.header.title).html(theme.header.title).appendTo(logo);
                }
            }
            if (theme.themes) {
    ...
    function init(options) {
        ...
        RED.i18n.init(options, function() {
            document.documentElement.lang = RED.i18n.lang();
