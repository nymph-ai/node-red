...
RED.view = (function() {
    ...
    function showTouchMenu(obj,pos) {
        RED.contextMenu.show({type:'workspace', x:pos[0], y:pos[1]});
        resetMouseVars();
    }
    ...
    function isCanvasTextNode(d) {
        return d && d.type === "canvas-text";
    }

    function clampCanvasTextNumber(value, fallback, min, max) {
    ...
    function getCanvasTextSize(d) {
        return {
            width: clampCanvasTextNumber(d.width, 520, 160, 1200),
            height: clampCanvasTextNumber(d.height, 260, 80, 900)
        };
    }

    function getCanvasTextValue(d) {
        if (d.canvasText && d.canvasText.hasOwnProperty("text")) {
            return String(d.canvasText.text);
        }
        return d.placeholder || "";
    }

    function wrapCanvasText(text, maxChars, maxLines) {
        var sourceLines = String(text || "").replace(/\r\n/g, "\n").replace(/\r/g, "\n").split("\n");
        var lines = [];
        var truncated = false;
        ...
        for (var i = 0; i < sourceLines.length; i++) {
            ...
            while (line.length > 0) {
                ...
                var slice = line.substring(0, maxChars);
            ...
            if (lines.length >= maxLines) {
                truncated = truncated || i < sourceLines.length - 1 || line.length > 0;
                break;
            }
        }
        ...
        if (truncated && lines.length > 0) {
            ...
            lines[lines.length - 1] = (last.length >= maxChars ? last.substring(0, Math.max(0, maxChars - 3)) : last) + "...";
        }
        return lines;
    }

    function getCanvasTextClipId(d) {
        return "red-ui-flow-node-canvas-text-clip-" + String(d.id).replace(/[^a-zA-Z0-9_-]/g, "-");
    }

    function formatCanvasTextLength(length) {
    ...
    function formatCanvasTextTime(updatedAt) {
        if (!updatedAt) {
            return "idle";
        }
    ...
    function updateCanvasTextNode(d, nodeEl) {
        if (!nodeEl || !nodeEl.__canvasTextGroup__) {
            return;
        }
        if (!isCanvasTextNode(d)) {
            nodeEl.__canvasTextGroup__.style.display = "none";
            return;
        }

        var hasValue = !!(d.canvasText && d.canvasText.hasOwnProperty("text"));
        var textValue = getCanvasTextValue(d);
        var truncated = !!(d.canvasText && d.canvasText.truncated);
        var length = d.canvasText && Number.isFinite(d.canvasText.length) ? d.canvasText.length : textValue.length;
        var property = d.canvasText && d.canvasText.property ? d.canvasText.property : (d.property || "payload");
        var iconOnRight = (!d._def.align && d.inputs !== 0 && d.outputs === 0) || d._def.align === "right";
        var iconInset = d._def.icon ? 38 : 10;
        var leftInset = iconOnRight ? 12 : iconInset;
        var rightInset = iconOnRight ? iconInset : 10;
        var headerHeight = 30;
        var footerHeight = 18;
        var bodyX = leftInset;
        var bodyY = headerHeight + 7;
        var bodyWidth = Math.max(1, d.w - leftInset - rightInset);
        var bodyHeight = Math.max(1, d.h - headerHeight - footerHeight - 12);
        var lineHeight = 13;
        var clipX = bodyX;
        var clipY = bodyY;
        var clipWidth = bodyWidth;
        var clipHeight = bodyHeight;
        var maxChars = Math.max(8, Math.floor(clipWidth / 6));
        var maxLines = Math.max(1, Math.floor(clipHeight / lineHeight));
        var lines = wrapCanvasText(textValue, maxChars, maxLines);
        var clipId = getCanvasTextClipId(d);

        nodeEl.__canvasTextGroup__.style.display = "inline";
        nodeEl.__canvasTextHeader__.style.display = "none";

        nodeEl.__canvasTextBodyRect__.style.display = "none";

        nodeEl.__canvasTextSeparator__.setAttribute("d", "M " + leftInset + " " + headerHeight + " H " + (d.w - rightInset));
        nodeEl.__canvasTextSeparator__.setAttribute("stroke", "#111111");
        nodeEl.__canvasTextSeparator__.setAttribute("stroke-opacity", "0.18");

        nodeEl.__canvasTextStatusDot__.style.display = "none";

        nodeEl.__canvasTextTitle__.textContent = RED.utils.getNodeLabel(d, d.type) || "canvas text";
        nodeEl.__canvasTextTitle__.setAttribute("x", leftInset);
        nodeEl.__canvasTextTitle__.setAttribute("y", 20);
        nodeEl.__canvasTextTitle__.setAttribute("font-size", "13px");
        nodeEl.__canvasTextTitle__.setAttribute("font-weight", "300");
        nodeEl.__canvasTextTitle__.setAttribute("fill", "#1b1b1b");
        nodeEl.__canvasTextTitle__.setAttribute("fill-opacity", "0.76");
        nodeEl.__canvasTextTitle__.removeAttribute("font-family");

        nodeEl.__canvasTextMeta__.textContent = formatCanvasTextTime(d.canvasText && d.canvasText.updatedAt);
        nodeEl.__canvasTextMeta__.setAttribute("x", d.w - rightInset);
        nodeEl.__canvasTextMeta__.setAttribute("y", 20);
        nodeEl.__canvasTextMeta__.setAttribute("font-size", "10px");
        nodeEl.__canvasTextMeta__.setAttribute("font-weight", "300");
        nodeEl.__canvasTextMeta__.setAttribute("fill", "#1b1b1b");
        nodeEl.__canvasTextMeta__.setAttribute("fill-opacity", hasValue ? "0.65" : "0.45");
        nodeEl.__canvasTextMeta__.removeAttribute("font-family");

        nodeEl.__canvasTextClip__.setAttribute("id", clipId);
        nodeEl.__canvasTextClipRect__.setAttribute("x", clipX);
        nodeEl.__canvasTextClipRect__.setAttribute("y", clipY);
        nodeEl.__canvasTextClipRect__.setAttribute("width", clipWidth);
        nodeEl.__canvasTextClipRect__.setAttribute("height", clipHeight);

        nodeEl.__canvasTextBody__.setAttribute("x", clipX);
        nodeEl.__canvasTextBody__.setAttribute("y", clipY + 10);
        nodeEl.__canvasTextBody__.setAttribute("font-size", "11px");
        nodeEl.__canvasTextBody__.setAttribute("font-weight", "300");
        nodeEl.__canvasTextBody__.setAttribute("fill", "#1b1b1b");
        nodeEl.__canvasTextBody__.setAttribute("fill-opacity", hasValue ? "0.68" : "0.45");
        nodeEl.__canvasTextBody__.setAttribute("clip-path", "url(#" + clipId + ")");
        nodeEl.__canvasTextBody__.removeAttribute("font-family");

        nodeEl.__canvasTextFooter__.textContent = property + (truncated ? "  truncated" : "");
        nodeEl.__canvasTextFooter__.setAttribute("x", leftInset);
        nodeEl.__canvasTextFooter__.setAttribute("y", d.h - 8);
        nodeEl.__canvasTextFooter__.setAttribute("font-size", "10px");
        nodeEl.__canvasTextFooter__.setAttribute("font-weight", "300");
        nodeEl.__canvasTextFooter__.setAttribute("fill", "#1b1b1b");
        nodeEl.__canvasTextFooter__.setAttribute("fill-opacity", "0.52");
        nodeEl.__canvasTextFooter__.removeAttribute("font-family");

        while (nodeEl.__canvasTextBody__.firstChild) {
            nodeEl.__canvasTextBody__.removeChild(nodeEl.__canvasTextBody__.firstChild);
        }
        for (var i = 0; i < lines.length; i++) {
            var tspan = document.createElementNS("http://www.w3.org/2000/svg", "tspan");
            tspan.setAttribute("x", clipX);
            tspan.setAttribute("dy", i === 0 ? 0 : lineHeight);
            tspan.textContent = lines[i];
            nodeEl.__canvasTextBody__.appendChild(tspan);
        }
    }
    ...
    function _redraw() {
        ...
        if (forceFullRedraw || showAllLinkPorts !== -1 || mouse_mode != RED.state.JOINING) {
            ...
            nodeEnter.each(function(d,i) {
                ...
                var canvasTextGroup = document.createElementNS("http://www.w3.org/2000/svg","g");
                canvasTextGroup.setAttribute("class","red-ui-flow-node-canvas-text");
                canvasTextGroup.style.display = "none";
                canvasTextGroup.style["pointer-events"] = "none";
                node[0][0].__canvasTextGroup__ = canvasTextGroup;

                var canvasTextHeader = document.createElementNS("http://www.w3.org/2000/svg","rect");
                canvasTextHeader.setAttribute("class","red-ui-flow-node-canvas-text-header");
                canvasTextHeader.setAttribute("rx",4);
                canvasTextHeader.setAttribute("ry",4);
                canvasTextGroup.appendChild(canvasTextHeader);
                node[0][0].__canvasTextHeader__ = canvasTextHeader;

                var canvasTextBodyRect = document.createElementNS("http://www.w3.org/2000/svg","rect");
                canvasTextBodyRect.setAttribute("class","red-ui-flow-node-canvas-text-body-rect");
                canvasTextBodyRect.setAttribute("rx",3);
                canvasTextBodyRect.setAttribute("ry",3);
                canvasTextGroup.appendChild(canvasTextBodyRect);
                node[0][0].__canvasTextBodyRect__ = canvasTextBodyRect;

                var canvasTextSeparator = document.createElementNS("http://www.w3.org/2000/svg","path");
                canvasTextSeparator.setAttribute("class","red-ui-flow-node-canvas-text-separator");
                canvasTextSeparator.setAttribute("stroke-width","1");
                canvasTextGroup.appendChild(canvasTextSeparator);
                node[0][0].__canvasTextSeparator__ = canvasTextSeparator;

                var canvasTextStatusDot = document.createElementNS("http://www.w3.org/2000/svg","circle");
                canvasTextStatusDot.setAttribute("class","red-ui-flow-node-canvas-text-status");
                canvasTextGroup.appendChild(canvasTextStatusDot);
                node[0][0].__canvasTextStatusDot__ = canvasTextStatusDot;

                var canvasTextTitle = document.createElementNS("http://www.w3.org/2000/svg","text");
                canvasTextTitle.setAttribute("class","red-ui-flow-node-canvas-text-title");
                canvasTextTitle.setAttribute("font-size","13px");
                canvasTextTitle.setAttribute("font-weight","300");
                canvasTextTitle.setAttribute("fill","#1b1b1b");
                canvasTextGroup.appendChild(canvasTextTitle);
                node[0][0].__canvasTextTitle__ = canvasTextTitle;

                var canvasTextMeta = document.createElementNS("http://www.w3.org/2000/svg","text");
                canvasTextMeta.setAttribute("class","red-ui-flow-node-canvas-text-meta");
                canvasTextMeta.setAttribute("font-size","10px");
                canvasTextMeta.setAttribute("font-weight","300");
                canvasTextMeta.setAttribute("text-anchor","end");
                canvasTextMeta.setAttribute("fill","#64748b");
                canvasTextGroup.appendChild(canvasTextMeta);
                node[0][0].__canvasTextMeta__ = canvasTextMeta;

                var canvasTextClip = document.createElementNS("http://www.w3.org/2000/svg","clipPath");
                var canvasTextClipRect = document.createElementNS("http://www.w3.org/2000/svg","rect");
                canvasTextClip.appendChild(canvasTextClipRect);
                canvasTextGroup.appendChild(canvasTextClip);
                node[0][0].__canvasTextClip__ = canvasTextClip;
                node[0][0].__canvasTextClipRect__ = canvasTextClipRect;

                var canvasTextBody = document.createElementNS("http://www.w3.org/2000/svg","text");
                canvasTextBody.setAttribute("class","red-ui-flow-node-canvas-text-body");
                canvasTextBody.setAttribute("font-size","11px");
                canvasTextBody.setAttribute("font-weight","300");
                canvasTextBody.setAttribute("fill","#1b1b1b");
                canvasTextGroup.appendChild(canvasTextBody);
                node[0][0].__canvasTextBody__ = canvasTextBody;

                var canvasTextFooter = document.createElementNS("http://www.w3.org/2000/svg","text");
                canvasTextFooter.setAttribute("class","red-ui-flow-node-canvas-text-footer");
                canvasTextFooter.setAttribute("font-size","10px");
                canvasTextFooter.setAttribute("font-weight","300");
                canvasTextFooter.setAttribute("fill","#64748b");
                canvasTextGroup.appendChild(canvasTextFooter);
                node[0][0].__canvasTextFooter__ = canvasTextFooter;

                nodeContents.appendChild(canvasTextGroup);
            ...
            node.each(function(d,i) {
                ...
                if (d.dirty) {
                    ...
                    var isLink = (d.type === "link in" || d.type === "link out")
                    var hideLabel = d.hasOwnProperty('l')?!d.l : isLink;
                    var isCanvasText = isCanvasTextNode(d);
                    if (isCanvasText) {
                        hideLabel = false;
                    }
                    dirtyNodes[d.id] = d;
                    ...
                    if (d.resize || this.__hideLabel__ !== hideLabel || this.__label__ !== label || this.__outputs__.length !== d.outputs) {
                        if (isCanvasText) {
                            labelParts = { lines: [], width: 0 };
                            d.resize = true;
                            this.__label__ = label;
                            this.__labelLineCount__ = 0;
                        } else {
                            labelParts = getLabelParts(label, "red-ui-flow-node-label");
                            if (labelParts.lines.length !== this.__labelLineCount__ || this.__label__ !== label) {
                                d.resize = true;
                        ...
                        if (isCanvasText) {
                            var canvasTextSize = getCanvasTextSize(d);
                            d.h = canvasTextSize.height;
                        } else if (hideLabel) {
                    ...
                    if (mouse_mode != RED.state.MOVING_ACTIVE) {
                        ...
                        if (this.__iconGroup__) {
                            this.__iconGroup__.style.display = "";
                        }
