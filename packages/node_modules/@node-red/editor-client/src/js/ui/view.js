...
RED.view = (function() {
    ...
    function updateCanvasTextNode(d, nodeEl) {
        ...
        var footerHeight = 22;
        var bodyX = 8;
        var bodyY = headerHeight + 6;
        var bodyWidth = Math.max(1, d.w - (bodyX * 2));
        var bodyHeight = Math.max(1, d.h - headerHeight - footerHeight - 14);
        var lineHeight = 14;
        var bodyPadding = 8;
        var clipX = bodyX + bodyPadding;
        var clipY = bodyY + bodyPadding;
        var clipWidth = Math.max(1, bodyWidth - (bodyPadding * 2));
        var clipHeight = Math.max(1, bodyHeight - (bodyPadding * 2));
        var maxChars = Math.max(8, Math.floor(clipWidth / 7));
        ...
        nodeEl.__canvasTextHeader__.setAttribute("x", 1);
        nodeEl.__canvasTextHeader__.setAttribute("y", 1);
        nodeEl.__canvasTextHeader__.setAttribute("width", Math.max(0, d.w - 2));
        nodeEl.__canvasTextHeader__.setAttribute("height", headerHeight);
        nodeEl.__canvasTextHeader__.setAttribute("fill", hasValue ? "#eaf6ff" : "#f1f5f9");

        nodeEl.__canvasTextBodyRect__.setAttribute("x", bodyX);
        nodeEl.__canvasTextBodyRect__.setAttribute("y", bodyY);
        nodeEl.__canvasTextBodyRect__.setAttribute("width", bodyWidth);
        nodeEl.__canvasTextBodyRect__.setAttribute("height", bodyHeight);
        nodeEl.__canvasTextBodyRect__.setAttribute("fill", "#ffffff");
        nodeEl.__canvasTextBodyRect__.setAttribute("stroke", "#dbe4ee");

        nodeEl.__canvasTextSeparator__.setAttribute("d", "M 1 " + headerHeight + " H " + (d.w - 1));
        nodeEl.__canvasTextSeparator__.setAttribute("stroke", "#dbe4ee");

        nodeEl.__canvasTextStatusDot__.setAttribute("cx", padding + 2);
        nodeEl.__canvasTextStatusDot__.setAttribute("cy", 17);
        nodeEl.__canvasTextStatusDot__.setAttribute("r", 4);
        nodeEl.__canvasTextStatusDot__.setAttribute("fill", hasValue ? (truncated ? "#f59e0b" : "#10b981") : "#94a3b8");

        nodeEl.__canvasTextTitle__.textContent = RED.utils.getNodeLabel(d, d.type) || "canvas text";
        nodeEl.__canvasTextTitle__.setAttribute("x", padding + 14);
        nodeEl.__canvasTextTitle__.setAttribute("y", 21);

        nodeEl.__canvasTextMeta__.textContent = formatCanvasTextTime(d.canvasText && d.canvasText.updatedAt);
        nodeEl.__canvasTextMeta__.setAttribute("x", d.w - padding);
        nodeEl.__canvasTextMeta__.setAttribute("y", 21);
        ...
        nodeEl.__canvasTextBody__.setAttribute("fill", hasValue ? "#0f172a" : "#64748b");
        nodeEl.__canvasTextBody__.setAttribute("clip-path", "url(#" + clipId + ")");

        nodeEl.__canvasTextFooter__.textContent = property + " · " + formatCanvasTextLength(length) + (truncated ? " · truncated" : "");
        nodeEl.__canvasTextFooter__.setAttribute("x", padding);
        nodeEl.__canvasTextFooter__.setAttribute("y", d.h - 7);
    ...
    function _redraw() {
        ...
        if (forceFullRedraw || showAllLinkPorts !== -1 || mouse_mode != RED.state.JOINING) {
            ...
            nodeEnter.each(function(d,i) {
                ...
                canvasTextTitle.setAttribute("font-size","12px");
                canvasTextTitle.setAttribute("font-weight","600");
                canvasTextTitle.setAttribute("fill","#111827");
                ...
                canvasTextBody.setAttribute("font-family","ui-monospace, SFMono-Regular, Menlo, Consolas, monospace");
                canvasTextBody.setAttribute("fill","#111827");
            ...
            node.each(function(d,i) {
                ...
                if (d.dirty) {
                    ...
                    if (mouse_mode != RED.state.MOVING_ACTIVE) {
                        ...
                        if (this.__iconGroup__) {
                            this.__iconGroup__.style.display = isCanvasText ? "none" : "";
                        }
